    ...
    if (params.column.colId === 'code_count') {
      if (checkValueIsEffectiveNum(params.newValue)) {
        if (Number(params.newValue) === 0) {
          errorMessage("代码量不能为0！")
        } else if ((params.newValue).toString().trim() === "") {
          // 如果输入空值，则返回为数据库原有值。
          newValues = {
            ...
            "method": "DELETE" // POST-修改, DELETE-删除
          }
        } else {
          newValues = {
            "execution": projectId, // 禅道执行ID
      ...
      newValues = {
        "execution": projectId, // 禅道执行ID
        "pkName": "quality", // 指标名称 - 项目质量
        "itmeName": "desc", // 指标项名称 - 代码量
        "tech": cuTech, // 技术侧: 1-前端, 2-后端
        "newValue": params.newValue, // 新修改值
        "method": "POST" // POST-修改, DELETE-删除
      }
    }
