    ...
    <PageContainer>
      ...
      <div>
        ...
        >
          ...
          <p style={cssIndent}>查需求的所属计划包含stage-patch字样的;</p>
          <p style={cssIndent}>且需求的‘发布引入版本’字段值不为空;</p>
          <p style={cssIndent}>
          ...
          <p>2.计算公式：</p>
          <p style={cssIndent}>
            不显示按人员（但要存人员的数据：完成任务的需求 和 需求任务指派给开发的）
          </p>
          <p style={cssIndent}>
            按部门：开发-发布引入patch数 = 指派给这个部门开发的需求数 +
            由这个部门开发完成的需求数（特殊说明‘指派给这个部门开发的需求数’：不是查需求的指派给，而是查需求关联任务的指派给；当1个需求有多个开发完成任务，给每个开发都算1个需求，若这里的多个开发都属于同一个部门时，需要对需求编号进行去重，注意区分一级部门、2级部门）;
          </p>
          <p style={cssIndent}>
