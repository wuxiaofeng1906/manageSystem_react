...
import {prjPlanDevRateRuleColumns, prjPlanDevRateRuleDatas} from "../devCommonRules";
    ...
    <PageContainer>
      ...
      <div>
        <Drawer title={<label style={{"fontWeight": 'bold', fontSize: 20}}>计算规则</label>}
          ...
          <p style={cssIndent}>1.周期：按周、按月、按季、按年统计（实际上线日期落在哪个周期，就算到对应周期）；</p>
          <p style={cssIndent}>2.只展示部门数据（不展示人员数据）；</p>
          <p style={cssIndent}>3.项目计划偏差率 = Average（各项目计划偏差率）；</p>
          <p style={cssIndent}> 单个项目计划偏差率 = (实际上线日期 - 计划上线日期)/(计划上线日期 - 计划开始日期）*100；</p>
          <p style={cssIndent}>4.项目取值范围：；</p>
          <Table columns={prjPlanDevRateRuleColumns} dataSource={prjPlanDevRateRuleDatas}
                 size={"small"} pagination={false} bordered/>
          <p style={cssIndent}>5.实际上线日期取值：任务类型为‘计划’，且任务名称包含'项目计划’，且任务状态为‘已完成 或 已关闭’ ，取该任务的‘实际完成日期’；</p>
          <p style={cssIndent}>6.计划上线日期取值：取执行概况中的'计划上线时间‘；</p>
          <p style={cssIndent}>7.计划开始日期取值：取概况，计划开始时间；</p>
