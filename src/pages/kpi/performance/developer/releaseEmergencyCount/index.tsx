...
const ReleaseEmergencyCounts: React.FC<any> = () => {
  ...
  const {data, loading} = useRequest(() =>
    queryEmergencyCounts(gqlClient, 'quarter'),
  );
    ...
    <PageContainer>
      ...
      <div>
        <Drawer title={<label style={{"fontWeight": 'bold', fontSize: 20}}>计算规则</label>}
          ...
          <p style={cssIndent}>且需求的'是否发布引入'字段值为'是'；当计划为空，是否发布引入为‘是’，查该需求关联的执行类型是否为’emergency’，是则算，不是则不算</p>
          <p style={cssIndent}>且需求关联的任务完成人/关闭人是开发，或需求关联的任务状态为未开始/进行中，查指派给为开发的；</p>
          <p>2.计算公式：</p>
          <p style={cssIndent}>不显示按人员</p>
          <p style={cssIndent}>按部门：开发-发布引入emergency数 = 指派给这个部门开发的需求数 + 由这个部门开发完成的需求数（特殊说明‘指派给这个部门开发的需求数’：不是查需求的指派给，而是查需求关联任务的指派给；当1个需求有多个开发完成任务，给每个开发都算1个需求，若这里的多个开发都属于同一个部门时，需要对需求编号进行去重，注意区分一级部门、2级部门）；</p>
          <p style={cssIndent}>按中心：开发-发布引入emergency数 = 就是根据1.取值范围中查出来的所有需求数；</p>
