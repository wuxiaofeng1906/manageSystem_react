...
import {queryCustomInputRate} from "./gridConfigure/data";

const CustomInputRate_A: React.FC<any> = () => {

  /* region ag-grid */
  const gridApi = useRef<GridApi>();
  const gqlClient = useGqlClient();
  const {data, loading} = useRequest(() =>
    queryCustomInputRate(gqlClient, 'quarter'),
  );
  ...
  const statisticsByWeeks = async () => {
    ...
    const datas: any = await queryCustomInputRate(gqlClient, 'week');
  ...
  const statisticsByMonths = async () => {
    ...
    const datas: any = await queryCustomInputRate(gqlClient, 'month');
    gridApi.current?.setRowData(datas);
  ...
  const statisticsByQuarters = async () => {
    ...
    const datas: any = await queryCustomInputRate(gqlClient, 'quarter');
    gridApi.current?.setRowData(datas);
  };
  ...
  const statisticsByYear = async () => {
    ...
    const datas: any = await queryCustomInputRate(gqlClient, 'year');
    gridApi.current?.setRowData(datas);
  };
    ...
    <PageContainer>
      ...
      <div>
        <Drawer title={<label style={{"fontWeight": 'bold', fontSize: 20}}>计算规则</label>}
          ...
          <p style={cssIndent}>且该任务的完成人/关闭人为测试；</p>
          <p style={cssIndent}>且任务有关联需求；</p>
          <p style={cssIndent}>且关联需求的’客户等级’值为A ，或 需求'客户等级'值不等于A，而该需求由bug转换而来，且该bug的‘客户等级值’为A的；</p>
          <p style={cssIndent}>注意：有主任务和子任务的，只取子任务的实际工时；</p>
          <p>2.计算公式：</p>
          <p style={cssIndent}>按人员：测试-A类客户服务投入率 = 该测试人员在该周期完成A类客户需求任务实际工时求和 / 该测试人员在该周期内所有工时之和 * 100；</p>
          <p style={cssIndent}>按部门：测试-A类客户服务投入率 = 该部门所有测试人员在该周期完成A类客户需求任务实际工时求和 / 该部门所有测试人员在该周期内所有工时之和 * 100；</p>
          <p style={cssIndent}>按中心：测试-A类客户服务投入率 = 该中心所有测试人员在该周期完成A类客户需求任务实际工时求和 / 该中心所有测试人员在该周期内所有工时之和 * 100；</p>
...
export default CustomInputRate_A;
