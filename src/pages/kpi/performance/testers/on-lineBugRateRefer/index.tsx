...
const queryOnlineBugRate = async (client: GqlClient<object>, params: string) => {
  ...
  const { data } = await client.query(`
      {
         newBugThousTestDept(kind: "${condition.typeFlag}", ends: ${condition.ends}, thous: REFER) {
            total {
              dept
              deptName
              kpi
  ...
  const datas = converseFormatForAgGrid(data?.newBugThousTestDept);
  return datas;
};
    ...
    <PageContainer>
      ...
      <div>
        ...
        >
          ...
          <p>
            <strong>3.bug统计范围</strong>
          </p>
          <p style={cssIndent}>
            3.1 产品为“1.0/1.0正式版--产品经理”“产品实施--实施顾问反馈”“开发自提需求”；{' '}
          </p>
          <p style={cssIndent}>3.2 创建者为“顾问”“产品”“客服”； </p>
          <p style={cssIndent}>
            3.3
            线上bug统计，统计os操作系统字段值包含“线上”的字段并除去线上bug-重复bug、线上bug-需求相关、线上bug-不是问题、线上bug-自动化发现；{' '}
          </p>
          <p style={cssIndent}>
            特殊判定条件： 当dept为3且os值为空时，仅查询项目名称包含hotfix字样的bug；
            当opendBy='fuyang.li'时，需增加条件bug创建时间&gt;=2020-04-01；{' '}
          </p>
          <p style={cssIndent}>3.4 解决方案为“空”“已解决”“延期处理”“后续版本”“代码未合并”的； </p>
          <p style={cssIndent}>
            3.5 统计人员,
            首先在zt_bug表中判断当前的assignedTo的值是不是测试，若是测试则该bug的用户名称则是该测试，
            若不是测试则在zt_action表中按降序挨个查id
            在zt_history表中的old字段按时间倒序去查测试人员，查到的第一个测试人员则该bug的用户名
            称则是该测试,若old中仍查不到测试人员，则倒序查zt_action.actor的值，查到的第一个测试人员则将该bug的用户名称记录为该测试，若还查不到
            测试人员则名称记录为空，并且人员的zt_user.dept in （35,36,37,38,39）；{' '}
          </p>
          ...
          <p style={cssIndent}>
            周报：周一至周天测试类有效bug求和/(当周周一至周天研发中心所有开发人员代码量之和/1000)；
          </p>
          <p style={cssIndent}>
