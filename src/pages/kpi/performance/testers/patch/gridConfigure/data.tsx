...
export const queryEmergencyCounts = async (client: GqlClient<object>, params: string) => {
  ...
  const { data } = await client.query(`
      {
         devTestReleasePatchDept(kind: "${condition.typeFlag}", ends: ${condition.ends}, identity:DEVELOPER) {
          total{
            dept
            deptName
            kpi
  ...
  const datas = converseForAgGrid_showDepts(data?.devTestReleasePatchDept);
  return datas;
};
/* endregion */
