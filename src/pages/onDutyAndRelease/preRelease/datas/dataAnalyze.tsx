...
// 数据校验： 上线环境是否为空
const checkOnlineEnvSource = (releaseItem: any, upgradeApi: any) => {
  const app = releaseItem.gridData?.some(
    (it: any) => !isEmpty(it.app) && isEmpty(it.online_environment),
  );
  const api = upgradeApi.gridData?.some(
    (it: any) => !isEmpty(it.api_name) && isEmpty(it.online_environment),
  );
  return app || api;
};
export { alalysisInitData, checkOnlineEnvSource };
