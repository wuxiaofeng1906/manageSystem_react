...
// 1级灰度发布列表(1级灰度 / 线上2-8集群进行中列表)
const getFirstGrayscaleListData = async (type = 'gray_one') => {
  const result: any = {
    message: '',
    data: [],
  };
  await axios
    .get(`/api/verify/release/${type}`, {
      params: {
        // release_method: releaseMethod,
        release_start_time: '',
        release_end_time: '',
...
const getFormalListData = async (condition: any) => {
  ...
  const data = {
    release_start_time: condition.start ? `${condition.start} 00:00:00` : '',
    release_end_time: condition.end ? `${condition.end} 23:59:59` : '',
    project_id: condition.project,
    page: condition.page,
    page_size: condition.pageSize,
  };
  await axios
    .get('/api/verify/release/online', { params: data })
    .then(function (res) {
      if (res.data.code === 200) {
...
const vertifyOnlineProjectExit = async (releaseMethod: string) => {
  ...
  await axios
    .get('/api/verify/release/online_app', { params: { release_type: releaseMethod } })
    .then(function (res) {
...
// 一键正式发布
const releaseOnline = async (
  onlineReleaseNum: string,
  releaseNums: string,
  releaseType: string,
) => {
