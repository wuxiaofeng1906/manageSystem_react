...
const PreReleaseList = ({disabled, height}: { disabled?: boolean; height: number }) => {
  ...
  const onFinish = async (data: any) => {
    if (!isEmpty(data)) {
      let href = `/onlineSystem/releaseOrder/${data?.release_num}/${data?.release_name}`;
      if (data.type == '1') {
        href = `/onlineSystem/prePublish/${data?.release_num}/${data?.branch}/${data?.release_name}`;
      }
      history.push(href);
    }
    setVisible(false);
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      setTabsLocalStorage({
                        ...
                        "is_delete": p.data.is_delete,
                        "release_type": p.data.release_type,
                      });
                      history.push(href);
                    }}
                  >
