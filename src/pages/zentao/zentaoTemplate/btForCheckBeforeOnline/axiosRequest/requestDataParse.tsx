...
import { queryDutyCardInfo } from '@/pages/onDuty/dutyPlan/data/axiosApi';
...
const loadExcutionSelect = async () => {
  ...
  if (excution && excution.length > 0) {
    excution.forEach((ele: any) => {
      excutionData.push(
        <Option
          key={ele.execution_id}
          sprintType={ele.sprint_type}
          value={`${ele.execution_id}&${ele.execution_name}`}
        >
          {ele.execution_name}
        </Option>,
      );
...
const getChildTaskPersonForPrj = (newDts: any, assignedTo: any) => {
  if (newDts.task_name.toString().includes('前端】')) {
    if (assignedTo.front && assignedTo.front.user_name) {
      // 有值班人员才取值，没有的话还是使用原始值。
